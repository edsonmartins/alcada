...
**Gate primário do piloto** — critério de saída e método de observação em `docs/PILOTO.md §1/§5`.
Pergunta anterior a tudo: o gestor **já viu o N2 rodar**? Se não, o piloto vem antes de F3; a resposta
vem do uso real, não da suíte de testes.
...
**Como medir sem baseline manual exaustivo:** ver `docs/PILOTO.md §4` — triangulação de quatro fontes
baratas (escape como piso, auditoria amostral do descarte, reconciliação de sexta, dupla-codificação
de 1 dia). **Pré-requisito:** medir G7 exige a **extração real** (OpenRouter, G9); com o gateway sem
chave, todo item entra em baixa confiança e só se mede o filtro de relevância, não o recall (PILOTO §6).
